...
function addChuanhuButton(botElement) {
  ...
  if (!rawMessage) {
    // 如果没有 raw message，说明是早期历史记录，去除按钮
  ...
  function removeMarkdownText(message) {
    ...
    if (rawDiv) {
      ...
      if (!rawDiv.querySelector("pre")?.classList.contains("fake-pre")) {
        rawDiv.innerHTML = rawDiv.innerHTML.replace(
          /<pre>/g,
          '<pre class="fake-pre">',
        );
      }
...
function setLatestMessage() {
  var latestMessage = gradioApp().querySelector(
...
function addLatestMessageButtons(botElement) {
  ...
  regenerateButton.innerHTML =
    regenIcon + `<span>${i18n(regenerate_i18n)}</span>`;
  ...
  deleteButton.innerHTML =
    deleteIcon + `<span>${i18n(deleteRound_i18n)}</span>`;
