...
function setUploader() {
  ...
  var uploaderObserver = new MutationObserver(function (mutations) {
    var fileInput = null;
    var fileCount = 0;
    fileInput = gradioApp().querySelector(
      "#upload-index-file table.file-preview",
    );
...
function setCheckboxes() {
  chatbotSingleSessionCB = gradioApp().querySelector(
    'input[name="single-session-cb"]',
  );
  chatbotOnlineSearchCB = gradioApp().querySelector(
    'input[name="online-search-cb"]',
  );
  grSingleSessionCB = gradioApp().querySelector(
    "#gr-single-session-cb > label > input",
  );
  grOnlineSearchCB = gradioApp().querySelector(
    "#gr-websearch-cb > label> input",
  );
  ...
  events.forEach((sourceEvent) => {
    target.addEventListener(sourceEvent, function (targetEvent) {
      ...
      source.dispatchEvent(
        new Event(sourceEvent, { detail: targetEvent.detail }),
      );
...
function bgSelectHistory(a, b) {
  const historySelectorInput = gradioApp().querySelector(
    "#history-select-dropdown input",
  );
