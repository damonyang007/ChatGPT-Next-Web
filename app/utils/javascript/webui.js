...
function showMask(obj) {
  ...
  if (obj == "box") {
    ...
    chatbotArea
      .querySelector("#chatbot-input-more-area")
    ...
    if (document.querySelector(".chuanhu-top-mask")) {
      for (
        var i = 0;
        i < document.querySelectorAll(".chuanhu-top-mask").length;
        i++
      ) {
...
function checkChatbotWidth() {
  ...
  if (windowWidth > 768) {
    ...
    if (
      !chatbotArea.classList.contains("toolbox-open") &&
      chatbotArea.classList.contains("menu-open")
    ) {
...
function switchToolBoxTab(tabIndex) {
  let tabButtons = gradioApp().querySelectorAll(
    "#chuanhu-toolbox-tabs .tab-nav > button",
  );
