...
export async function upload_files(files) {
  const uploadInputElement = gradioApp().querySelector(
    "#upload-index-file > .center.flex input[type=file]",
  );
  let totalSizeMb = 0;
  if (files && files.length > 0) {
    ...
    if (uploadInputElement) {
      ...
      Object.defineProperty(uploadInputElement, "files", {
        value: files,
        enumerable: true,
      });
