...
function addInit() {
  ...
  uploaderIndicator = gradioApp().querySelector(
    '#upload-index-file > div[data-testid="block-label"]',
  );
  chatListIndicator = gradioApp().querySelector(
    "#history-select-dropdown > div.wrap",
  );
  ...
  chatbotObserver.observe(chatbotIndicator, {
    attributes: true,
    childList: true,
    subtree: true,
  });
...
function checkModel() {
  ...
  function checkGPT() {
    ...
    if (modelValue.toLowerCase().includes("gpt")) {
      ...
      gradioApp()
        .querySelector("#header-btn-groups")
        .classList.remove("is-gpt");
    }
...
function setScrollShadow() {
  const toolboxScroll = toolbox.querySelector(
  ...
  function adjustScrollShadow() {
    ...
    if (
      toolboxScroll.scrollLeft + toolboxScroll.clientWidth <
      toolboxScrollWidth
    ) {
      ...
      () => {
        ...
        if (!chatbotIndicator.classList.contains("translucent")) {
          ...
          var checkLatestAdded = setInterval(() => {
            var latestMessageNow = gradioApp().querySelector(
              "#chuanhu-chatbot > .wrapper > .wrap > .message-wrap .message.bot.latest",
            );
            if (
              latestMessageNow &&
              latestMessageNow.querySelector(".message-btn-row")
            ) {
