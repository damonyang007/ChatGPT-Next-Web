...
var isLatestVersion =
  localStorage.getItem("isLatestVersion") === "true" || false;
var shouldCheckUpdate = false;

function setUpdater() {
  const enableCheckUpdate = gradioApp().querySelector(
  ...
  shouldCheckUpdate =
    !updateInfoGotten &&
    ((!isLatestVersion && longTimeNoCheck) || isLatestVersion);
    ...
    if (mutation.type === "attributes" || mutation.type === "childList") {
      if (statusDisplay.innerHTML.includes('id="update-status"')) {
        if (getUpdateStatus() === "success") {
          ...
          updatingInfoElement.innerHTML = marked.parse(updateFailure_i18n, {
            mangle: false,
            headerIds: false,
          });
...
async function updateLatestVersion() {
  ...
  const currentVersion =
    versionMatch && versionMatch[1].length == 8 ? versionMatch[1] : null;
  ...
  const localVersionTime =
    versionTime !== "unknown" ? new Date(versionTime).getTime() : 0;
  disableUpdateBtns();
    ...
    if (currentVersion) {
      ...
      if (latestVersionTime) {
        ...
        if (localVersionTime == 0) {
          ...
          versionInfoElement.innerHTML = marked.parse(infoMessage, {
            mangle: false,
            headerIds: false,
          });
          console.log(`New version ${latestVersion} found!`);
          ...
          noUpdate(
            "Local version check failed, it seems to be a local rivision. <br>But your revision is newer than the latest release.",
          );
          gradioApp().classList.add("is-outdated");
          enableUpdateBtns();
          localStorage.setItem("isLatestVersion", "false");
...
function getUpdateInfo() {
  window.open(
    "https://github.com/gaizhenbiao/chuanhuchatgpt/releases/latest",
    "_blank",
  );
  closeUpdateToast();
}
...
function bgUpdateChuanhu() {
  ...
  try {
    updateSpinner = new Spin.Spinner({
      color: "#06AE56",
      top: "45%",
      lines: 9,
    }).spin(updatingSpinner);
  } catch (error) {
